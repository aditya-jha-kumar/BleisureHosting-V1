...
import { FaMapMarkerAlt, FaCalendarAlt, FaUser, FaArrowRight, FaPlus } from "react-icons/fa";
    ...
    <div className={`relative w-full h-96 ${className}`}>
      ...
      <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2 w-full max-w-3xl">
        <div className="bg-white p-4 rounded-lg shadow-lg flex flex-col sm:flex-row sm:space-y-0 space-y-4 sm:space-x-4">
          ...
          {/* Filter inputs, visible only when the filter bar is open on small screens */}
          <div className={`sm:flex flex-col sm:flex-row sm:space-y-0 space-y-4 sm:space-x-4 ${isFilterBarOpen ? "block" : "hidden sm:flex"}`}>
